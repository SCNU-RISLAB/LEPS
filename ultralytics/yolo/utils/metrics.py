...
class WIoU_Scale:
    ''' monotonous: {
        ...
        需要设置scale，scale为True时，WIoU会乘以一个系数，结合monotonous即会对应WIoU的3个版本。
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=True, SIoU=False, EIoU=False, WIoU=False,
                     Focal=False, alpha=1, gamma=0.5, scale=False, eps=1e-7, MPDIoU=False):
    """
    计算bboxes iou
    GIoU: 为True时计算GIoU v5自带
    DIoU: 为True时计算DIoU v5自带
    CIoU: 为True时计算CIoU v5自带，默认使用
    SIoU: 为True时计算SIoU
    EIoU: 为True时计算EIoU
    WIoU: 为True时计算WIoU
    MPDIou: 为True时计算WIoU
    ...
    gamma: Focal_XIoU中的gamma参数，默认为0
    .5
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU or WIoU or MPDIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
