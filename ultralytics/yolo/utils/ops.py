...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS修改前
        i = soft_nms(boxes, scores, iou_thres)  # NMS修改后2

        i = i[:max_det]  # limit detections
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            # Update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
            from ultralytics.yolo.utils.metrics import box_iou
            #iou = box_iou(boxes[i], boxes) > iou_thres  # iou matrix 修改前
            iou = box_iou_for_nms(boxes[i], boxes, GIoU=False, DIoU=False, EIou=False, CIoU=False, SIoU=False, eps=1e-7) > iou_thres  # iou matrix修改后2
